	://nodejs.cn/api/   Node.js 中文网  v12.16.1
...
把bin放入Path中 如下.exe就一个文件 
 ...
 ^  会把当前库的版本更新到当前major version（也就是第一位数字）中最新的版本。
	  如 bluebird:^3.3.4，这个库会去匹配3.x.x中最新的版本，但是他不会自动更新到4.0.0。
 ~  他会更新到当前minor version（也就是中间的那位数字）中最新的版本。
	  如	body-parser:~1.15.2，这个库会去匹配更新到1.15.x的最新版本，如果出了一个新的版本为1.16.0，则不会自动升级
 *  一般是最后一次正式发布版本
...
=========Cordova 最新版本 Electron 做为替代,web技术开发跨三大平台桌面应用，使用Python2.7.x
-----Electron
